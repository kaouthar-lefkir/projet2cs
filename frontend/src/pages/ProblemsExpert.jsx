...
import SolutionPage from "./SolutionExpert"; // Import SolutionManager

export default function ProblemsManager({ onBackClick }) {
  // Sample data for recent problems - moved to useState to make it updatable
  ...
  // Handle updating a solution for a problem
  const handleSolutionUpdate = (updatedSolution) => {
    // Find the problem in our state and update its solution
    const updatedProblems = allProblems.map(problem => {
    ...
    // Update the selected problem to reflect the changes
    setSelectedProblem({
      ...selectedProblem,
      solution: updatedSolution
    });
  ...
  const handleSolutionDelete = () => {
    // Find the problem in our state and set its solution to null
    const updatedProblems = allProblems.map(problem => {
      if (problem.id === selectedProblem.id) {
        return {
          ...problem,
  ...
  // If a problem is selected, show the solution page
  if (selectedProblem) {
    return (
      <SolutionPage 
        ...
        onSolutionDelete={handleSolutionDelete}
      />
    );
  }
            ...
            {currentProblems.map((problem, index) => (
              <tr key={index} className="border-t border-gray-200">
                ...
                <td className="py-4 px-3 text-right">
                  ...
                  >
                    {problem.solution ? "View Solution" : "Solution"}
                  </button>
                </td>
              </tr>
            ))}
